...
export default function PV(props){
  ...
  const PVMenu = () => {
    return (
      <nav style={{display:"flex", backgroundColor:"#001529", height:"5vh", }} >
        <div style={pv_menu_font_style}><ReUploadPDBButton onClickHandle={()=>{props.setIfStruc(false)}} style={pv_menu_item_font_styles}/></div>
  ...
  const PVViewer = () =>{
    useEffect(() => {
      ...
      console.log(props.StrucPDBPath);
      
      var pdb_filename = '/pdbs/'+Path2Filename(props.StrucPDBPath)
      var parent = document.getElementById("pvviewer"+props.method)
          ...
          parent.addEventListener('mousemove', function(event) {
            ...
            if (picked !== null) {
              try {
                ...
                document.getElementById("residue_info"+props.method).innerHTML = atom.residue().name() + atom.residue().num();
                var color = [0,0,0,0];
                ...
                document.getElementById("residue_info"+props.method).innerHTML = picked._legacyObject.userData;
              }
            } else {
              prevPicked = null;
              document.getElementById("residue_info"+props.method).innerHTML = '&nbsp;';
            }
            viewer.requestRedraw();
          });

          if(PVpaths!=='' && PVpaths!==null && PVpaths!==undefined){
    ...
    },[props.StrucPDBPath, PVcolor, PVstyle, PVpaths,PVlabel,PVlabelColor, PVpathStartColor, PVpathEndColor, PVpathMiddleColor, PVpathTubeColor]);
    return (

      <>
      <div id={"pvviewer"+props.method} style={{height:"45vh", width:"100%", position:"relative"}}>
        <div id={"residue_info"+props.method} style={{position: "absolute", top:"1vh", right:"1vh", color:"white", fontSize:"1.5vh"}}></div>
      </div>
      </>
  ...
  return (
    <div style={{width:"100%", position:"relative"}} ref={resizeRef} >
      <PVMenu />
      <PVViewer/>
    </div>
  )
